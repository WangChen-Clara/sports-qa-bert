...
if __name__ == '__main__':
    ...
    if cf.do_predict:
        ...
        with open('D:\pythonProject\ES_baesd_QA_sport\sentence1.txt','r') as s:
            sentence1 = s.read()
        # sentence1 = '今天晚上要不去吃西餐吧？'
        with open(r'D:\pythonProject\ES_baesd_QA_sport\responses.json', 'r') as f:
            a = json.load(f)
        sentence2=[]
        for i in a:
            sentence2.append(i.get("sim_question"))
        print(sentence2)
        # sentence2 = ['当预算只有十万元时，买哪种车最好？', '忽忽几晨昏，离别间之，疾病间之，不及终年同静好', '上海自来水来自海上','今天晚上去吃什么？']
        t = 0
        for i in sentence2:
            predict = sim.predict(sentence1, i)
        ...
        # print(answer)
        if t<0.5:
            print("对不起，这个问题没有找到合适答案。")
            # 当相似度小于0.5则没有匹配到相似问题
        else:
            index = sentence2.index(answer)
            print("answer:", a[index]['answers'])
